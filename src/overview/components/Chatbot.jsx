...
export default function Chatbot() {
  ...
  useEffect(() => {
    const fetchConversationId = async () => {
      try {
        const res = await axios.get("http://localhost:5000/api/chatbot/conversation-id", {
  ...
  const fetchHistory = async (conversationId, limit = 5, offsetVal = 0, appendToTop = false) => {
    try {
      const res = await axios.get(
        `http://localhost:5000/api/chatbot/chat/history?conversationId=${conversationId}&limit=${limit}&offset=${offsetVal}`,
        { withCredentials: true }
      );
    ...
    try {
      ...
      if (!conversationId && newConversationId) {
        ...
        const historyRes = await axios.get(
          `http://localhost:5000/api/chatbot/chat/history?conversationId=${newConversationId}&limit=5&offset=0`,
          { withCredentials: true }
        );
